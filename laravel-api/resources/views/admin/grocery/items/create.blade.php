...
@section('admin_content')
  <div class="bg-white rounded-xl shadow-sm">
    <div class="px-6 py-4 border-b border-gray-100">
      <h2 class="text-lg font-semibold text-gray-900">Create Item</h2>
    </div>
    <div class="p-6">
      <form method="POST" action="/admin/grocery/items" enctype="multipart/form-data">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select name="category_id" class="w-full h-12 px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-[#f04848] focus:outline-none" required>
              ...
              <option value="{{ $c->id }}" @selected(old('category_id')==$c->id)>{{ $c->title }}</option>
              @endforeach
            </select>
