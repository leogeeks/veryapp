  ...
  <form method="POST" action="/admin/tasks/{{ $task->id }}">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div>
        ...
        <input name="title" value="{{ old('title', $task->title) }}" class="mt-1 w-full rounded-lg border-gray-300 focus:border-primary focus:ring-primary" required>
      ...
      <div class="md:col-span-2">
        ...
        <textarea name="description" class="mt-1 w-full rounded-lg border-gray-300 focus:border-primary focus:ring-primary" rows="4">{{ old('description', $task->description) }}</textarea>
        @error('description')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700">Due Date</label>
        <input type="datetime-local" name="due_date" value="{{ old('due_date', optional($task->due_date)->format('Y-m-d\TH:i')) }}" class="mt-1 w-full rounded-lg border-gray-300 focus:border-primary focus:ring-primary">
