...
@section('admin_content')
  ...
  <form method="POST" action="/admin/users/{{ $user->id }}">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        ...
        <input name="name" value="{{ old('name', $user->name) }}" class="mt-1 w-full rounded-lg border-gray-300 focus:border-primary focus:ring-primary" required>
        @error('name')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700">Email</label>
        <input type="email" name="email" value="{{ old('email', $user->email) }}" class="mt-1 w-full rounded-lg border-gray-300 focus:border-primary focus:ring-primary" required>
        @error('email')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700">New Password (optional)</label>
        <input type="password" name="password" class="mt-1 w-full rounded-lg border-gray-300 focus:border-primary focus:ring-primary">
  ...
  <form method="POST" action="/admin/users/{{ $user->id }}" class="mt-6" onsubmit="return confirm('Delete user?')">
    @csrf
    @method('DELETE')
    <button class="px-4 py-2 rounded-lg bg-red-600 text-white font-semibold hover:bg-red-700">Delete User</button>
  </form>
@endsection
