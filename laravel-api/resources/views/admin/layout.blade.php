      ...
      <div class="px-4 sm:px-6 lg:px-8">
        <div class="flex h-16 items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <a href="/admin/dashboard" class="text-lg font-semibold text-gray-900">Admin</a>
          </div>
          <div class="flex items-center gap-3">
            <span class="text-sm text-gray-600 hidden sm:block">{{ auth()->user()->name ?? 'Guest' }}</span>
            <form action="/admin/logout" method="POST">
              @csrf
              <button class="inline-flex items-center px-3 py-1.5 rounded bg-gray-900 text-white text-sm hover:bg-gray-800">Logout</button>
            </form>
          </div>
        </div>
      </div>
    ...
    <div class="flex">
      <aside class="w-64 bg-white border-r border-gray-200 hidden md:block min-h-[calc(100vh-4rem)]">
        <nav class="p-4 space-y-1">
          <a href="/admin/dashboard" class="block px-3 py-2 rounded hover:bg-gray-100">Dashboard</a>
